	...
	public class MovableBase : IMovable
	{
		...
		public MovableBase(GameModel model, string ImageFile, int X, int Y)
		{
			...
			if (!SkinIgnoreDirection)
			{
				...
				goUp.Add(Useful.GetImageByName(ImageFile + "Up"));
				goDown.Add(Useful.GetImageByName(ImageFile + "Down"));
				goRight.Add(Useful.GetImageByName(ImageFile + "Right"));
				goLeft.Add(Useful.GetImageByName(ImageFile + "Left"));
			}
			else
			{
